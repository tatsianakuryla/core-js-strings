...
function removeFirstOccurrences(str, value) {
  ...
  const stringBegin = str.slice(0, startIndex);
  const stringEnd = str.slice(startIndex + valueLength);
  const returnedString = stringBegin + stringEnd;

  return str.includes(value) ? returnedString : str;
}
...
function isPalindrome(str) {
  const newStr = str.toLowerCase().replace(/[^\w\s]|_/g, '');
  const newStrTrimmed = newStr.replaceAll(' ', '');
  return newStrTrimmed.split('').reverse().join('') === newStrTrimmed;
}
...
function findLongestWord(sentence) {
  ...
  return wordsArray.reduce((longest, word) => {
...
function reverseWords(str) {
  const strArray = str.split(' ');
  const newStr = strArray.map((word) => word.split('').reverse().join(''));
...
function invertCase(str) {
  ...
  const resultArray = strArray.map((letter) => {
 ...
 *
 * @example
 *   getStringFromTemplate('John',
 * 'Doe') => 'Hello, John Doe!'
 *   getStringFromTemplate('Chuck',
 * 'Norris') => 'Hello, Chuck Norris!'
 */
